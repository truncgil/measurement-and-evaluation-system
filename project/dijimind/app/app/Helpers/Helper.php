...
function profile_pic($size=128,$u="") {
    if($u=="") {
        $u = u();
    }
    
    if($u->file!="") {
        return picture2($u->file,$size,0);
    } else {
        return url("assets/user.png");
    }
}

function file_get_contents2($fn) {
...
function people_chart($total,$value,$color="default") {
    if($value>100) $value=100;
    if($total>100) $total=100;
    $fark = $total - $value;
    if($fark<1) $fark = 1;
    if($fark>100) $fark = 100;
    
   
     ?>
     <div class="people-chart">
        <?php for($k=1;$k<=$value;$k++) { ?>
            <i class="fa fa-user text-<?php echo $color ?>"></i>
        <?php } ?>
        <?php for($k=1;$k<=$fark;$k++) { ?>
            <i class="far fa-user text-<?php echo $color ?>"></i>
        <?php } ?>
     </div>
     <?php 
}
...
function cevap_dizi() {
    return explode(",","A,B,C,D,E");
}

function branslar() {
    $brans = db("contents")->select("id","title","json","title2","slug")->where("type","Branşlar")->get();
    return $brans;
}
/*
...
*/
function soru($id) { //bir soruya ait bilgileri döndürür
    $soru = db("soru_bankasi")
    ->where("id",$id)
    ->first();
    return $soru;
}
function uzman_cevap() { // uzmanın cevapladıkları
    $cevaplar = db("soru_atama")->where("user",u()->id)->get();
    $cevap_dizi = array();
    foreach($cevaplar AS $oc) {
        /*
        $cevap_dizi[$oc->soru_id] = [
            "id" => $oc->soru_id,
            'tak_duzey'=>$oc->tak_duzey,
            'cevap'=>$oc->cevap
        ];
        */
        array_push($cevap_dizi,$oc->soru_id);
    }
    return $cevap_dizi;
}   
...
function soru_atama() { //uzmana atanmış sorular
    $yetkiler = db("soru_atama")
    ->where("user",u()->id)
    ->whereNull("cevap")
    ->get("soru_id");
    $dizi = array();
    foreach($yetkiler AS $y) {
        array_push($dizi,$y->soru_id);
    }
    return $dizi;
}
function chart_count($title,$count,$icon="bag",$class="col-lg-2 col-md-4") {
     ?>
<div class="{{$class}}">
    <a class="block block-link-shadow text-right" href="javascript:void(0)">
        <div class="block-content block-content-full clearfix">
            <div class="float-left mt-10 d-none d-sm-block">
                <i class="si si-{{$icon}} fa-3x text-body-bg-dark"></i>
            </div>
            <div class="font-size-h3 font-w600" data-toggle="countTo" data-speed="1000" data-to="{{$count}}">0</div>
...
function soru_atama2() { //uzmana atanmış soruların cevaplanmış hali
    $yetkiler = db("soru_atama")
    ->where("user",u()->id)
    ->whereNotNull("cevap")
    ->get("soru_id");
...
function sb_sinavlar() { //soru bankasındaki tüm sınavları getirir
    $sinavlar = db("soru_bankasi")
    ->whereNotNull("group")
    ->groupBy("group")
...
function sb_sinavlar2() { //soru bankasındaki tüm sınavları getirir
    $sinavlar = db("soru_bankasi")
    ->whereNotNull("group")
    ->where("uid",u()->id)
    ->groupBy("group")
...
function atama_sayisi($id) {
    $atamalar = db("soru_atama")->where("soru_id",$id)->count();
    return $atamalar;
}
function atanan_cevaplayan_sayisi($id) {
    $atamalar = db("soru_atama")->where("soru_id",$id)->whereNotNull("cevap")->count();
    return $atamalar;
}
function kazanimlar() {
    $brans = branslar();
    ...
    foreach($brans AS $b) {
        $j = j($b->json);
        if(!isset($kazanimlar[$b->title])) $kazanimlar[$b->title]= [];
        if(is_array($j)) {
           // print2($j); exit();
            $konular = explode("\n",$j['Konular']);
...
function mailsend($to="",$subject="",$html="",$link="",$linkyazi="Tıklayınız",$attach="") {
    ...
    $title = "Dijimind";
    try {
        Mail::send("mail-template", $data, function($message) use($to, $subject,$title,$attach){
        
            $message->from("noreply@dijimindakademi.com", $title);
            $message->to($to);
...
function picture2($f,$size=512,$storage=1) {
    if($storage==1) {
...
function nf($price,$type="",$decimal=2) {
    /*
    $price = str_replace(",","",$price);
    $price = str_replace(".","",$price);
    $price = str_replace("$","",$price);
...
function c($slug)
{
    $c = Contents::where("slug", $slug)->orWhere("id", $slug)->orderBy("id","DESC")->first();
    return $c;
}
...
{
    return Contents::where("y", 1)
        ->where(function($query) use($type){
            $query->orWhere("kid",$type);
...
function implode_key($glue,$array) {
    $dizi = array();
    foreach($array AS $alan => $deger) {
        array_push($dizi,$alan);
    }
    return implode($glue,$dizi);
}

function aliasUsersArray() {
    $alias = u()->alias;
    $users = db("users")->where("alias",$alias);
    $users = $users->get();
...
{   
    ...
    if(Auth::check()) {
        ...
        $ogrenciler = db("users")->where("level","Öğrenci")->where(function($query) use($u){
            $query = $query->orWhere("anne",$u->phone);
            $query = $query->orWhere("baba",$u->phone);
        })->get();
        if($ogrenciler) {
            $ogrenci_dizi = [];
            foreach($ogrenciler AS $o) {
                array_push($ogrenci_dizi,$o->id);
            }
            $u['ogrenci_ids'] = $ogrenci_dizi;
            $u['ogrenciler'] = $ogrenciler;
        }
